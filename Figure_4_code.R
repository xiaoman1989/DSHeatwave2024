...
setwd("D:/地湖所工作/春季热浪文章相关工作/XMGS02_new2/Figure_proportion")

rawdata <- read_excel(
  "Cell_all.xlsx",
  col_types = c("skip","text","skip","text", "skip","numeric",
                "text","text","text","text")
)
...
## ======================= 修改：重新定义处理组标签 =======================
# 新的处理组分类
treatment_labels_new <- c(
  "57" = "Non-heatwave + Non-mixing",   # C
  "63" = "Non-heatwave + Non-mixing",   # N
  "54" = "Non-heatwave + Mixing",       # M
  "59" = "Non-heatwave + Mixing",       # M
  "68" = "Non-heatwave + Mixing",       # M + N
  "70" = "Non-heatwave + Mixing",       # M + N
  "62" = "Heatwave + Non-mixing",       # H + N
  "67" = "Heatwave + Non-mixing",       # H + N
  "64" = "Heatwave + Mixing",           # H + M + N
  "65" = "Heatwave + Mixing"            # H + M + N
)
...
treatment_short_labels <- c(
  "Non-heatwave + Non-mixing" = "C/N",
  "Non-heatwave + Mixing" = "M",
  "Heatwave + Non-mixing" = "H+N",
  "Heatwave + Mixing" = "H+M+N"
)
...
total_cells_daily <- rawdata %>%
  ...
  summarise(
    total_cells = sum(Cell_ml, na.rm = TRUE),
    .groups = "drop"
  )
...
phylum_summary <- rawdata %>%
  ...
  summarise(
    cells_mean = mean(Cell_ml, na.rm = TRUE),  # 门内物种平均
    cells_sum = sum(Cell_ml, na.rm = TRUE),    # 门内物种总和（用于计算比例）
...
cell_change_stats <- cell_change_stats %>%
  mutate(
    Treatment_Group = treatment_labels_new[as.character(Tank)]
  )
...
phylum_treatment_summary <- phylum_proportion_stats %>%
  ...
  # 按照指定的顺序排列处理组
  mutate(Treatment_Group = factor(Treatment_Group, levels = treatment_order)) %>%
  arrange(Treatment_Group, Latin_phylum)
...
phylum_diversity_stats <- phylum_data %>%
  ...
  # 按照指定的顺序排列处理组
  mutate(Treatment_Group = factor(Treatment_Group, levels = treatment_order)) %>%
  arrange(Treatment_Group, Tank)

## ======================= 保存统计结果到Excel =======================
output_folder <- "Figure_proportion_260325"
...
writeData(wb, "Total_Cell_Statistics", 
          cell_change_stats %>% 
            mutate(Treatment_Group = factor(treatment_labels_new[as.character(Tank)], 
                                            levels = treatment_order)) %>%
            arrange(Treatment_Group, Tank) %>%
            select(Tank, Treatment, Treatment_Group, 
...
daily_data <- total_cell_stats %>%
  ...
  select(Tank, Treatment, Treatment_Group, Day, Concentration) %>%
  mutate(Treatment_Group = factor(Treatment_Group, levels = treatment_order)) %>%
  arrange(Treatment_Group, Tank, Day) %>%
  pivot_wider(
    id_cols = c(Tank, Treatment, Treatment_Group),
    names_from = Day,
...
writeData(wb, "Daily_Concentration", daily_data,
          startRow = 1, startCol = 1)
setColWidths(wb, "Daily_Concentration", cols = 1:13, widths = "auto")

# 4. 门类比例详细统计（每个tank）
addWorksheet(wb, "Phylum_Proportion_Detailed")
phylum_proportion_stats_sorted <- phylum_proportion_stats %>%
  mutate(Treatment_Group = factor(Treatment_Group, levels = treatment_order)) %>%
  arrange(Treatment_Group, Tank, desc(as.numeric(str_extract(Day8_proportion, "[0-9.]+"))))
writeData(wb, "Phylum_Proportion_Detailed", phylum_proportion_stats_sorted,
          startRow = 1, startCol = 1)
...
phylum_data_sorted <- phylum_data %>%
  mutate(Treatment_Group = treatment_labels_new[as.character(Tank)]) %>%
  mutate(Treatment_Group = factor(Treatment_Group, levels = treatment_order)) %>%
  arrange(Treatment_Group, Tank, Day, Latin_phylum)
writeData(wb, "Phylum_Raw_Data", phylum_data_sorted,
          startRow = 1, startCol = 1)
setColWidths(wb, "Phylum_Raw_Data", cols = 1:ncol(phylum_data_sorted), widths = "auto")
...
## ======================= 修改：重新定义Tank顺序（按处理组分组） =======================
# 按处理组重新排列Tank顺序
tank_order_new <- c(
  # Non-heatwave + Non-mixing (C, N)
  57, 63,
  # Non-heatwave + Mixing (M, M, M+N, M+N)
  54, 59, 68, 70,
  # Heatwave + Non-mixing (H+N, H+N)
  62, 67,
  # Heatwave + Mixing (H+M+N, H+M+N)
  64, 65
)
...
phylum_data <- phylum_data %>%
  mutate(
    Tank = factor(Tank, levels = tank_order_new),
    Treatment_Group = treatment_labels_new[as.character(Tank)],
    Treatment_Short = treatment_short_labels[Treatment_Group]
  )
...
panel_labels <- data.frame(
  Tank = factor(tank_order_new, levels = tank_order_new),
...
# ===== 2. 主图（修改点：将geom_line改为geom_point）=====
p <- ggplot(phylum_data) +
  ...
  # 修改：使用geom_point替代geom_line，形状为菱形（shape=18）
  geom_point(
    aes(
      x = factor(Day),
      y = total_cells,
...
geom_text(
  ...
  facet_wrap(
    ...
    labeller = as_labeller(function(x) {
      tank_num <- as.numeric(as.character(x))
      treatment_group <- treatment_labels_new[as.character(tank_num)]
      treatment_group
    })
  ) +
...
cat("\n处理组重新分类：\n")
cat("  - Non-heatwave + Non-mixing: Tanks 57, 63 (C, N)\n")
cat("  - Non-heatwave + Mixing: Tanks 54, 59, 68, 70 (M, M, M+N, M+N)\n")
cat("  - Heatwave + Non-mixing: Tanks 62, 67 (H+N, H+N)\n")
cat("  - Heatwave + Mixing: Tanks 64, 65 (H+M+N, H+M+N)\n")