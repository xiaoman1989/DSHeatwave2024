...
# 创建响应变量标签映射（用于图表展示）
response_labels <- c(
  "total_biovolume" = "Total biovolume",
  ...
  "Fv_Fm" = "Fv/Fm",
  "Chlorophyta_bio" = "Chlorophyta biovolume",
  "Chlorophyta_cells" = "Chlorophyta cell cocentration",
  "Chlorophyta_Ratio" = "Chlorophyta Ratio",
  "Cyanophyceae_bio" = "Cyanophycea biovolume",
  "Cyanophyceae_cells" = "Cyanophyceae cell concentration",
  "Cyanophyceae_Ratio" = "Cyanophyceae Ratio",
  "filamentous_ratio" = "filamentous ratio",
  "filamentous_cells" = "filamentous cell concentration",
  "bloom_ratio" = "bloom ratio",
  "bloom_cells" = "bloom cell concentration",
  "Pseudanabaena_sp_cells" = "Pseudanabaena sp. cell concentration",
  "Pseudanabaena_sp_Ratio" = "Pseudanabaena sp. Ratio",
  "Closterium_sp_bio" = "Closterium sp. biovolume",
  "Closterium_sp_cells" = "Closterium sp. cell concentration",
  "Closterium_sp_Ratio" = "Closterium sp. Ratio",
  "Chlorella_sp_bio" = "Chlorella sp. biovolume",
  "Chlorella_sp_cells" = "Chlorella sp. cell concentration",
  "Chlorella_sp_Ratio" = "Chlorella sp. Ratio",
  "Scenedesmus_sp_Ratio" = "Scenedesmus sp. Ratio",
  "Kirchneriella_sp_bio" = "Kirchneriella sp. biovolume",
  "Kirchneriella_sp_Ratio" = "Kirchneriella sp. Ratio",
  "Cryptomonas_sp_bio" = "Cryptomonas sp. biovolume"
)
  ...
  if (nrow(selected_models) > 0) {
    ...
    if (length(beta_results) > 0) {
      ...
      for (y in selected_models$Response) {
        ...
        # 修改1: Adjusted R2改为小数点后两位
        coef_df <- coef_df %>%
          mutate(
            Response = y,
            # 添加响应变量标签
            Response_Label = response_labels[y],
            Adjusted_R2 = round(model_summary_y$adj.r.squared, 2),  # 改为小数点后两位
        ...
        p <- ggplot(coef_df, aes(x = Beta, y = reorder(Variable, Beta), fill = Beta)) +
          ...
          labs(
            title = paste0(coef_df$Response_Label[1], " | Adjusted R² = ", sprintf("%.2f", coef_df$Adjusted_R2[1])),  # 使用两位小数格式
